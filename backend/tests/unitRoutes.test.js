...
// categoryRoutes.test.js, using the Unit model and the public
// /api/units resource. See that file for the full rationale
...
import { Unit } from '../src/models/classificationModel.js';
...
describe('units resource', () => {
  ...
  describe('GET /api/units', () => {
    test('rejects an unauthenticated request', async () => {
      const res = await request(testApp()).get('/api/units');
      assert.equal(res.status, 401);
      assert.equal(res.body.error.code, 'UNAUTHORIZED');
    });

    test('returns an empty list for a user with no units', async () => {
    ...
    test('excludes archived units by default', async () => {
      await Unit.create([
        { _id: 'unit-active', ownerId: OWNER_A, name: 'Test Name', archived: false, isDefault: false, updatedAt: new Date() },
        { _id: 'unit-archived', ownerId: OWNER_A, name: 'Old', archived: true, isDefault: false, updatedAt: new Date() }
      ]);
      ...
      assert.equal(res.body[0]._id, 'unit-active');
    });

    test('includes archived units when includeArchived=true', async () => {
      await Unit.create([
        { _id: 'unit-active', ownerId: OWNER_A, name: 'Test Name', archived: false, isDefault: false, updatedAt: new Date() },
        { _id: 'unit-archived', ownerId: OWNER_A, name: 'Old', archived: true, isDefault: false, updatedAt: new Date() }
      ]);
      ...
      const res = await request(testApp())
        .get('/api/units?includeArchived=true')
        .set(authHeader(OWNER_A));
    ...
    test('ownership isolation: owner A cannot see owner B\'s units', async () => {
      await Unit.create({ _id: 'unit-b', ownerId: OWNER_B, name: 'B\'s unit', archived: false, isDefault: false, updatedAt: new Date() });
  ...
  describe('PUT /api/units/:id', () => {
    ...
    test('creates a new unit', async () => {
      ...
      assert.equal(res.status, 200);
      assert.equal(res.body._id, 'unit-1');
      ...
      const stored = await Unit.findById('unit-1').lean();
      assert.ok(stored);
      assert.equal(stored.ownerId, OWNER_A);
    });

    test('updates an existing unit (idempotent id -> exactly one document)', async () => {
      await request(testApp()).put('/api/units/unit-1').set(authHeader(OWNER_A)).send({ name: 'Test Name' });
      const res = await request(testApp())
        .put('/api/units/unit-1')
        .set(authHeader(OWNER_A))
      ...
      const count = await Unit.countDocuments({ _id: 'unit-1' });
    ...
    test('rejects a body.id that mismatches the URL id with VALIDATION_ERROR', async () => {
      const res = await request(testApp())
        .put('/api/units/unit-a')
        .set(authHeader(OWNER_A))
        .send({ id: 'unit-b', name: 'Electronics' });

      assert.equal(res.status, 400);
      assert.equal(res.body.error.code, 'VALIDATION_ERROR');

      const stored = await Unit.findById('unit-a').lean();
    ...
    test('ownership isolation: owner A cannot modify owner B\'s unit', async () => {
      await Unit.create({ _id: 'unit-b', ownerId: OWNER_B, name: 'B\'s unit', archived: false, isDefault: false, updatedAt: new Date() });

      const res = await request(testApp())
        .put('/api/units/unit-b')
        .set(authHeader(OWNER_A))
        .send({ name: 'Hijacked' });
      ...
      const stillB = await Unit.findById('unit-b').lean();
      assert.equal(stillB.name, 'B\'s unit');
      ...
      const count = await Unit.countDocuments({ _id: 'unit-b' });
      assert.equal(count, 1);
    });

    test('different owners can independently create units using different ids without interfering with each other', async () => {
      ...
      const resA = await request(testApp())
        .put('/api/units/owner-a-independent-id')
        .set(authHeader(OWNER_A))
        .send({ name: 'Owner A Unit' });
      const resB = await request(testApp())
        .put('/api/units/owner-b-independent-id')
        .set(authHeader(OWNER_B))
        .send({ name: 'Owner B Unit' });

      assert.equal(resA.status, 200);
      assert.equal(resB.status, 200);

      const all = await Unit.find({}).lean();
